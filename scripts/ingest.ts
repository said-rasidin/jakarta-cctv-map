...
async function main() {
  ...
  for (const [id, grouped] of groups) {
    ...
    if (override && withinJakarta(override.lat, override.lng)) coordinates = { lat: override.lat, lng: override.lng, source: "manual" };
    else if (prior?.coordinates && withinJakarta(prior.coordinates.lat, prior.coordinates.lng)) coordinates = { lat: prior.coordinates.lat, lng: prior.coordinates.lng, source: "manual" };
    ...
    const site: CameraSite = {
      ...
      agency: first.agency, provider: "Bali Tower", address: null, catalogSource: "jakarta-public", coordinates,
      ...
      channels: grouped.map((channel) => {
        ...
        return {
          ...
          playback: { kind: url ? "hls" : "iframe", url, embedUrl: channel.embedUrl, corsCapture: "unknown", checkedAt: null, aiEligible: Boolean(url) },
        };
      }),
    };
    sites.push(site);
  ...
  const dataset: CameraDataset = { schemaVersion: 2, generatedAt: new Date().toISOString(), sourceUrl: SOURCE_URL, unresolvedCount: unresolved.length, sites: sites.sort((a, b) => a.name.localeCompare(b.name, "id")) };
