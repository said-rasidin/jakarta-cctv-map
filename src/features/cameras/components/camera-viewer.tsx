...
export function CameraViewer({
  ...
  const [detectionControlsElement, setDetectionControlsElement] =
    useState<HTMLDivElement | null>(null);
  const active =
    site?.channels.find((channel) => channel.id === activeChannelId) ??
    site?.channels[0];
  ...
  useEffect(() => {
    if (!open || !site) return;
    setActiveChannelId(
      site.channels.some((channel) => channel.id === initialChannelId)
        ? initialChannelId!
        : (site.channels[0]?.id ?? ""),
    );
  }, [initialChannelId, open, site]);

  useEffect(() => {
    if (!open || !active || !active.embedUrl) {
      setHealth("unknown");
    ...
    const timer = window.setTimeout(
      () =>
        setLoadState((value) => (value === "loading" ? "unavailable" : value)),
      12000,
    );
    fetch(`/api/stream-health?channelId=${encodeURIComponent(active.id)}`, {
      signal: controller.signal,
    })
      .then((response) => response.json())
      .then((body: { status: StreamHealth }) => {
        setHealth(body.status);
        if (body.status === "unavailable") setLoadState("unavailable");
      })
      .catch(() => undefined);
    return () => {
      controller.abort();
  ...
  const handleVideo = useCallback((element: HTMLVideoElement | null) => {
    setVideo(element);
    if (element) setLoadState("loaded");
  }, []);
  ...
  const showIframe =
    hasStream && (!showDirect || fallback) && loadState !== "unavailable";
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 z-50 max-h-[92vh] overflow-auto rounded-t-3xl border border-slate-700 bg-slate-900 p-4 shadow-2xl outline-none md:inset-y-0 md:right-0 md:left-auto md:w-[min(680px,56vw)] md:rounded-none md:border-y-0 md:border-r-0">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <Dialog.Description className="mt-1 text-sm text-slate-400">
                {site.agency} · {site.district ?? "DKI Jakarta"}
              </Dialog.Description>
            </div>
            <Dialog.Close
          ...
          >
            ...
            >
              {site.channels.map((channel) => (
                <Tabs.Trigger
                  key={channel.id}
                  value={channel.id}
                  className="rounded-full border border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-300 data-[state=active]:border-sky-400 data-[state=active]:bg-sky-400/15 data-[state=active]:text-sky-200"
                >
                  {channel.label}
                </Tabs.Trigger>
              ))}
            ...
            >
              ...
              {showIframe && (
                <iframe
                  key={`${active.id}-${reloadKey}-fallback`}
              ...
              {showDirect && AI_FEATURE_ENABLED && (
                <DetectionOverlay
                  video={video}
                  eligible={Boolean(playback?.aiEligible)}
                  generationKey={`${active.id}-${reloadKey}`}
                  controlsTarget={detectionControlsElement}
                />
              )}
              {loadState === "loading" && !showDirect && (
                <div className="pointer-events-none absolute inset-0 grid place-items-center bg-black/35 text-sm text-slate-200">
                  Menghubungkan ke siaran…
                </div>
              )}
              {loadState === "unavailable" && (
                  ...
                  <div>
                    <p className="font-medium text-white">
                      {hasStream
                        ...
                        : "Stream direct belum tersedia"}
                    </p>
                    <p className="mt-2 text-sm text-slate-400">
                      {hasStream
                        ? "Coba lagi atau buka halaman sumber secara langsung."
                        : "Lokasi tersedia di peta; buka katalog sumber untuk informasi kamera."}
                    </p>
                  </div>
                </div>
              )}
          ...
          {showDirect && AI_FEATURE_ENABLED && (
            <div ref={setDetectionControlsElement} className="mt-3" />
          )}
          <p className="mt-3 text-xs text-slate-500">
            {fallbackReason ||
              (hasStream
                ? `Status koneksi: ${health === "available" ? "tersedia" : health === "unavailable" ? "tidak tersedia" : "belum dapat dipastikan"}`
                : "Status koneksi: tidak ada URL Bali Tower yang terverifikasi.")}
          </p>
          {showDirect && AI_FEATURE_ENABLED && (
            <p className="mt-1 text-xs text-slate-500">
              AI hanya menganalisis kamera yang sedang dibuka dan berhenti saat
              pemutar ditutup.
            </p>
          )}
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-400"
            >
              <ExternalLink size={16} />{" "}
              {hasStream ? "Buka tautan sumber" : "Buka katalog sumber"}
            </a>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
