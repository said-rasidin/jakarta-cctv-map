...
try {
  ...
  await expect(page.getByLabel("Cari kamera monitor")).toBeHidden();
  await expect(page.getByLabel("Nama susunan")).toBeHidden();
  await page.getByRole("button", { name: "Atur", exact: true }).click();
  await expect(page.getByLabel("Cari kamera monitor")).toBeVisible();
  if (!(await page.evaluate(() => window.__firstVideo.isConnected))) throw new Error("Settings recreated video");
  ...
  await expect(page.getByLabel("Mode waktu")).toHaveValue("aligned");
  await page.getByRole("button", { name: "Selesai mengatur", exact: true }).click();
  ...
  await page.screenshot({ path: "out/monitor-redesign-grid.png", fullPage: true });
  ...
  await page.getByText("Opsi kamera", { exact: true }).click();
